...
def DistanciaMonofasica(Linea, Nodo, Fases):
    ...
    if Fases == 0:
        ...
        res=np.linalg.inv(F)*V
        D=res[0,0]
        print('Distancia de falla fase B '+str(D))
        return D
    elif Fases == 2:
...
def Localizacion(Vpre, Ipre,VPost, IPost, TaNodos, TaLineas, TaCargas, Tipo, Fase):
    ...
    print(' va: '+str(np.abs(Va))+' vb: '+str(np.abs(Vb))+' vc: '+str(np.abs(Vc))+' Ia: '+str(np.abs(Ia))+ ' Ib: '+str(np.abs(Ib))+' Ic: '+str(np.abs(Ic)))
    print('PostFalla va: '+str(np.abs(VaPost))+' vb: '+str(np.abs(VbPost))+' vc: '+str(np.abs(VcPost))+' Ia: '+str(np.abs(IaPost))+ ' Ib: '+str(np.abs(IbPost))+' Ic: '+str(np.abs(IcPost)))
    ...
    print('Inicio corriente de prefalla')
    princpal=0
    ...
    for t in range(3):
        ...
        #for i in range(len(Cargas)):
            #print('Corriente de Carga '+str(i+1)+': '+str(np.abs(Cargas[i].I)))
        #######################################################################
        #                   Calculo de corrientes                             #
        #######################################################################
        ...
        Linea2='NaN'
        carga1='NaN'
        #print('Entra a calculo de corriente prefalla')
        while fin == 0:
            #print('Nodo Anterior: '+str(Nodos[actual].alias)+' ID de Nodo: '+str(Nodos[actual].id)+' Nodo Actual: '+str(Nodos[siguiente].alias)+' ID de Nodo: '+str(Nodos[siguiente].id)+ ' Nodo siguiente: '+str(Nodos[final].alias)+' ID de Nodo: '+str(Nodos[final].id))
            #print('Verificado nodo actual? '+str(Nodos[siguiente].ver)+' Nodo siguiente verificado? '+str(Nodos[final].ver))
            '''print('Inicio de los resultados de corriente de lineas')
            for i in range(len(Lineas)):
                print('Corriente de Linea '+str(i+1)+': '+str(np.abs(Lineas[i].I)))
            print('Fin de los resultados de corriente de lineas')'''
            ...
            #conectados
            #print('La linea 2 afuera del if es: '+str(Linea2))
            #print('¿Ya entro a Linea2?'+str(Lineas[Linea2].ver))
            if Lineas[Linea2].ver == 0 and len(Nodos[final].sig) != 0 and Nodos[final].ver == 0 and Nodos[siguiente].ver == 0:
                #print('Nodo Final con más nodos')
                actual = siguiente
                siguiente = final
            ...
            #conectados (linea conectada a un nodo final)
                ...
                #print('Nodo Final sin más nodos, asigno corriente')
                I=np.matrix([[0],[0],[0]])
                carga1='NaN'
                for i in range(len(Cargas)):
                ...
                if carga1 != 'NaN':
                    ...
                    Lineas[Linea2].I=I
                Lineas[Linea2].ver=1
                #print('La linea 2 dentro del if es: '+str(Linea2))
                Nodos[final].ver=1
                final1 = final
                final = 'NaN'
                for i in Nodos[siguiente].sig:
                ...
                #print('Nodo Final con otras lineas conectadas')
                ...
                Lineas[Linea2].I=I
                Lineas[Linea2].ver=1
                #print('La linea 2 dentro del if 2 es: '+str(Linea2))
                final1=final
                final = 'NaN'
                for i in Nodos[siguiente].sig:
                    if Nodos[i-1].ver == 0:
                ...
                #print('Nodo actual con corriente en la linea 1-')
                I=np.matrix([[0],[0],[0]])
                carga1='NaN'
                for i in range(len(Cargas)):
                    if Cargas[i].nodo == final +1:
            ...
            if terminar == 1 and Lineas[Linea1].ver == 1:
                ...
                fin = 1
        #print('Sale de calculo de corriente prefalla')
        for i in range(len(Nodos)):
            Nodos[i].ver=0
        for i in range(len(Lineas)):
            Lineas[i].ver=0
    ...
    #                                                                                #
    ##################################################################################
    print('Fin corriente de prefalla')
    ...
    if D > Lineas[LineaActual].D:
        #Si la falla no está en la primera línea
        anterior=None
        actual=principal
        siguiente=Lineas[LineaActual].nodo2 - 1
        terminar = 0
        #Se recorre todo el circuito hasta encontrar un posible punto de falla
        while terminar == 0:
            '''print('+++++++++++++++++++++++')
            print('Inicio')
            if anterior != None:
                print('Nodo anterior '+str(Nodos[anterior].alias))
            print('Nodo actual '+str(Nodos[actual].alias))
            print('Nodo siguiente '+str(Nodos[siguiente].alias))
            print('+++++++++++++++++++++++')'''
            LineaActual=None
            for i in range(len(Lineas)):
                if Lineas[i].nodo1 == actual+1 and Lineas[i].nodo2 == siguiente+1:
                    LineaActual=i
                    break
            if LineaActual == None:
                print('Error: No se encuentra linea Actual')
                return
            if Lineas[LineaActual].ver == 0:
                #print('----------------------')
                #print('Nodo anterior '+str(Nodos[anterior].alias))
                #print('Nodo actual '+str(Nodos[actual].alias))
                #print('Nodo siguiente '+str(Nodos[siguiente].alias))
                #print('----------------------')
                #Se busca la linea anterior
                for i in range(len(Lineas)):
                    if Lineas[i].nodo1 == anterior+1 and Lineas[i].nodo2 == actual+1:
                        LineaAnterior=i
                        break
                ######################################################################################
                #Se calcula la corriente de post-falla, basándose en la corriente de la línea anterior
                ######################################################################################
                ITemp=Lineas[LineaAnterior].IPost
                #Se busca si el nodo actual tiene una carga conectada
                CargasNodo=None
                for i in range(len(Cargas)):
                    if Cargas[i].nodo == actual + 1:
                        CargasNodo = i
                if CargasNodo != None:
                    ITemp=ITemp-Cargas[CargasNodo].I*Ibase
                #Se buscan las líneas paralelas a la linea actual y se restan las corrientes de prefalla
                for i in Nodos[actual].sig:
                    #Se busca los nodos siguientes que aún no están elegidos
                    if i-1 != siguiente:
                        siguienteTemp=i-1
                        #Se buscan las líneas paralelas
                        for l in range(len(Lineas)):
                            if Lineas[l].nodo1 == Nodos[actual].id and Lineas[l].nodo2 == Nodos[siguienteTemp].id:
                                #Se restan las corrientes de prefalla
                                ITemp = ITemp - Lineas[l].I*Ibase
                Lineas[LineaActual].IPost=ITemp
                #######################################
                #Se calcula el voltaje del nodo actual#
                #######################################
                Nodos[actual].tenPost= Nodos[anterior].tenPost - (Lineas[LineaAnterior].D*Lineas[LineaAnterior].Z*Lineas[LineaAnterior].IPost)
                ...
                #Si el nodo actual ya fue verificado, significa que ya se hizo un cálculo de distancia con ese nodo
                #Por lo tanto se avanza o retrocede en el circuito según el caso
                #Inicialmente se supone que se va a avanzar, pero finalmente se definirá esto
                #Si el existen nodos siguientes libres
                print('El nodo actual está verificado')
                if anterior != None:
                    print('Nodo anterior '+str(Nodos[anterior].alias))
                print('Nodo actual '+str(Nodos[actual].alias))
                print('Nodo siguiente '+str(Nodos[siguiente].alias))
                for i in range(len(Nodos)):
                        print('Nodo '+str(Nodos[i].id)+'-'+str(Nodos[i].alias)+' Verificado? '+str(Nodos[i].ver))
                        
                if Nodos[siguiente].ver == 0:
                    anterior=actual
                    actual=siguiente
                    siguiente = None
                    #print('Nodo RB2p está verificado? '+str(Nodos[20].ver))
                    #Se buscan nodos siguientes libres
                    #print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')
                    #print('Nodos Verificados')
                    #print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')
                    for i in Nodos[actual].sig:
                        if Nodos[i-1].ver==0:
                            siguiente=i-1
                ...
                if siguiente == None:
                    Nodos[siguiente1].ver=1
                    #Si no hay ningún nodo siguiente libre, se regresa en el circuito
                    #print('Nodo anterior '+str(anterior)+' nodo principal '+str(principal))
                    if anterior1 != principal:
                        #print('anterior es diferente de principal')
                        #Se verifica que el nodo anterior de este momento no se el nodo principal
                        siguiente=actual1
                        actual=anterior1
                        anterior = Nodos[anterior1].ant-1
                        #print(Nodos[anterior].alias)


                    else:
                        #print('anterior es igual a principal')
                        #Si es el nodo principal, significa que ya se retrocedió todo lo posible
                        #Y el cálculo acaba
                        #Antes de acabar, se verifica que todos los nodos se hayan recorrido
                        #Si no sucede esto, significa que ocurrió algún error
                        Verificar= 0
                        for m in range(len(Nodos)):
                            if Nodos[m].ver == 0:
                                #print('Nodo no verificado'+str(Nodos[m].alias))
                                Verificar = 0
                                break
                            else:
                                Verificar=1
                        if Verificar == 1:
                            ...
                            print('Error: Falla en la localizacion')
                            return
