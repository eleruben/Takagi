...
%        Z3p(k) = V3p(k)/I3p(k);    
%    end
    Ip0 = V./(Zsec(1,:)+Zsec(2,:)+Zsec(3,:)+3*Zf);
    Ip1 = V./(Zsec(1,:)+Zsec(2,:)+Zsec(3,:)+3*Zf)+ISE;
    Ia = 2*Ip0+Ip1;
    Iasin = 3*Ip0;
    Iap = Ia+3*k0.*Ip0;
    Iapsin = Iasin+3*k0.*Ip0;
    Vp1 = V(1)-Zsec(2,1)*Ip1;
    Vp2 = V(1)-Zsec(1,1)*Ip0;
    Vp0 = V(1)-Zsec(3,1)*Ip0;
    Va = Vp0+Vp1+Vp2;
    Z1p = Va./Iap;
    Z1psin = Va./Iapsin;
%    subplot(2,1,1)
%    plot(DistASE,abs(Z1p),'.',DistASE,abs(Z1psin),'r.')
%    subplot(2,1,2)
    axis manual
    plot( DistASE,imag(Z1p),'.',DistASE,imag(Z1psin),'r.')
    axis([0 14 0 0.05])
    pause
%    plot( imag(Zsec(2,:)),imag(Z1psin),'.')
%    axis([0 0.05 0 0.05])
%    pause
end
