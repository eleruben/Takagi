...
for k=1:nTramos
   ...
   NodP(k+2) = tramos(k).nodoP+1;
   NodQ(k+2) = tramos(k).nodoQ+1;
   z1(k+1) = tramos(k).Z012(2);
   z2(k+1) = tramos(k).Z012(3);
   z0(k+1) = tramos(k).Z012(1);
end
Ss = sum(Pinstal);
CargaSE = 0.6*(0.95+sin(acos(0.95))*j)*Ss/SBase;% cambio potencia
Perd = 0.025*CargaSE;
PerA = 0;
pasos=0;
for k=1:nTramos
...
%    end
    ...
    Ip0 = V(:,2:489)./(Zsec(1,:)+Zsec(2,:)+Zsec(3,:)+3*Zf);
    Ip1 =Ip0; 
    %Ip1 = V(:,2:489)./(Zsec(1,:)+Zsec(2,:)+Zsec(3,:)+3*Zf)+ISE;
    %Ia = 2*Ip1+Ip0;
    Iasin = 3*Ip0;
    Ia= Iasin;
    Vfp1 =V(:,2:489)-Zsec(2,1)*Ip1;
    Vfp2 = -Zsec(1,1)*Ip1;
    Vfp0 = -Zsec(3,1)*Ip0;
    Vfa = Vfp0+Vfp1+Vfp2;
